...
//    Initialize에서 Set_Sprite로 자기 스프라이트만 지정한다.
//  - 충돌(블락)은 별도 타일에서 처리 (여기서는 시각 배치만).
// ================================================================
...
protected:
    // 하위 클래스가 Initialize에서 호출.
    // key     : Img_Manager 등록 키 / cx,cy : 원본 픽셀 크기(스케일 1.0)
    // fHeight : 세로 보정(+ 위로) / fSortOff : 깊이정렬 보정
